...
@dataclass
class MetricContainer: #Chứa danh sách các metric
    ...
    prefix : str
        Prefix of metric names.     #Ví dụ như là val, train, test.
    """
    ...
    def __post_init__(self):
        self.metrics = Metric.get_metrics_by_names(self.metric_names) #Danh sách các lớp metric
        self.names = [self.prefix + name for name in self.metric_names] #Danh sách tên các metric
    ...
    def __call__(self, y_true, y_pred):
        ...
        """
        logs = {} #dict lưu trữ các metric với key: metric_name, value: metric_value
        for metric in self.metrics: #tính giá trị của metric
...
class Metric:
    ...
    @classmethod #Chuyển def thành class
    def get_metrics_by_names(cls, names):
        ...
        """
        available_metrics = cls.__subclasses__() #Danh sách lớp con (các lớp gọi lớp Metric)
        available_names = [metric()._name for metric in available_metrics] #Lấy tên của các lớp con
        metrics = []
        for name in names: #Kiểm tra tính hợp lệ của danh sách các metric được đưa vào
            assert (
        ...
        return metrics #Trả về danh sách của các lớp metric (metric class)
...
def check_metrics(metrics):
    ...
    for metric in metrics:
        if isinstance(metric, str):
            val_metrics.append(metric) #Nếu metric là str (tức là built-in metric) thì append vào
        elif issubclass(metric, Metric):
            val_metrics.append(metric()._name) #Nếu metric là class (tức là custom metric) thì append name vào
        else:
