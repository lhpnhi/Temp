...
#Tạo train và valid dataloaders
...
def validate_eval_set(eval_set, eval_name, X_train, y_train):
    ...
    #Kiểm tra xem kích thước eval_set có bằng kích thước eval_name không
    assert len(eval_set) == len(
        eval_name
    ), "eval_set and eval_name have not the same length"

    #Kiểm tra xem mỗi phần tử trong eval_set có bằng 2 hay không (vì mỗi phần tử là 1 tuple (X, y))
    if len(eval_set) > 0:
        assert all(
    ...
    for name, (X, y) in zip(eval_name, eval_set):
        #Kiểm tra tính hợp lệ của đầu vào X
        check_input(X)

        #Kiểm tra xem số chiều của X và X_train có như nhau không
        msg = (
        ...
        #Kiểm tra xem số chiều của y và y_train có như nhau không
        msg = (
        ...
        #Kiểm tra xem số đặc trưng (số cột) của X và X_train có như nhau không
        msg = (
        ...
        #Kiểm tra xem số lượng mẫu trong X và y có như nhau không
        msg = (
...
def define_device(device_name):
    """
    Xác định xem sẽ sử dụng "cpu" hay "gpu" để huấn luyện
    Parameters
    ----------
    device_name : str
...
def check_input(X):
    """
    Kiểm tra đầu vào, đưa ra thông báo lỗi nếu đầu vào là pandas dataframe. 
    Còn nếu không phải là pandas dataframe thì kiểm tra đầu vào dựa trên scikit learn (có dạng là (n_samples, n_features))
    """
